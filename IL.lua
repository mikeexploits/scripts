...
local guiTheme={
	...
	listBottomColor=c3(0.07,0.07,0.07)
}
...
local function reanimate()
	...
	local cammag=-v3Get((cfGet(camcf,"Position")-(pos+v3_010*1.5)),"Magnitude")
	pos=pos+v3_010*7
	...
	local joints={
		{
			...
			C0=cf(0,2,0,-1,0,0,0,0,1,0,1,-0),
			C1=cf(0,-1,0,-1,0,0,0,0,1,0,1,-0)
		},
		{
			Name="RootJoint",
		...
		},
		{
			...
			C0=cf(2,1,0,0,0,1,0,1,-0,-1,0,0),
			C1=cf(-1,1,0,0,0,1,0,1,-0,-1,0,0)
		},
		{
			Name="Left Shoulder",
		...
		},
		{
			...
			C0=cf(2,-2,0,0,0,1,0,1,-0,-1,0,0),
			C1=cf(1,2,0,0,0,1,0,1,-0,-1,0,0)
		},
		{
			Name="Left Hip",
	...
	local function mainFunction()
		...
		if raycastresult then
			...
			onground=v3Get(pos,"Y")-raycastresult<10.01
			if onground then
				Yvel=0
				pos=pos+v3_010*(raycastresult+10-v3Get(pos,"Y"))*min(delta*20,1)
				...
				if v3Get(pos,"Y")+Yvel*delta<raycastresult then
					Yvel=0
					pos=pos+v3_010*(raycastresult+10-v3Get(pos,"Y"))
				end
			end
		else
			Yvel=0
		...
		elseif shiftlock then	
			if isWalking then
				if walkSpeed==0 then
					...
					if onground then
						cfr=cfl(pos,pos+camcfLV*v3_101)
						camcf=cfAdd(camrot,pos+v3_030+camcfRV*1.75+camcfLV*cammag)
						cframes[rootpart]=cfr
						insSet(cam,"CFrame",camcf)
						lerpsIdle()
						noYvelTime=min(noYvelTime+delta*0.3,1)
			...
			if isWalking then
				if walkSpeed==0 then
					...
					if onground then
						cfr=cfAdd(cfGet(cfr,"Rotation"),pos)
						camcf=cfAdd(camrot,pos+v3_030+camcfLV*cammag)
						cframes[rootpart]=cfr
						insSet(cam,"CFrame",camcf)
						lerpsIdle()
						noYvelTime=min(noYvelTime+delta*0.3,1)
...
btn("Immortality Lord",function()
	...
	local flypos=nil
	local charcf=cf_0
	local function animations(onground)		
		TimingSine=sine*60
		local vel=getVel()
	...
	local function animations1()
		animations(true)
	end
	local function animations0()
		animations(false)
	end
